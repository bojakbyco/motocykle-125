...
export function scoreMotorcycle(m:Motorcycle,a:QuizAnswers) {
  ...
  const field=priorityField[a.priority]; const priority=field ? five(m.editorial[field] as number) : a.priority==='performance' ? Math.min(1,m.engine.powerKw/11) : a.priority==='cost' ? (m.price ? Math.max(.3,1-m.price.amount/25000) : .5) : .75;
